...
const { default: makeWASocket } = require("@fadzzzdigital-corp/baileys")
const { useMultiFileAuthState, DisconnectReason } = require('@fadzzzdigital-corp/baileys');
...
const memeg = {
  photoStart: 'https://files.catbox.moe/v2p7v5.mp4' // ganti dengan link hasil upload kamu
};
...
async function handleBioCheck(ctx, numbersToCheck) {
    ...
    if (numbersToCheck.length === 0) return ctx.reply("Nomornya mana??");

    await ctx.reply(`⏳  Sedang memproses ${numbersToCheck.length} nomor.`);
    ...
    let fileContent = "HASIL CEK BIO SEMUA USER BY @invydev_bot\n\n";
    ...
    await ctx.replyWithDocument({ source: filePath }, { caption: "✅Success Cek Bio" });
...
bot.command('start', (ctx) => {
  ...
  const caption = `Halo ${userName} ✨\nSemoga Bot Ini Bisa Membantu`;

  ctx.replyWithAnimation(
    { url: memeg.photoStart },
    {
      caption: caption,
      parse_mode: 'Markdown',
      ...Markup.inlineKeyboard([
        [
          Markup.button.callback('📜 Cek Bio', 'ckbio'),
          Markup.button.callback('👑 Connect Bot', 'pairv2')
        ],
        [Markup.button.callback('📂TOOLS', 'toolsv')]
      ])
    }
  );
});

bot.action('ckbio', (ctx) => {
    ctx.answerCbQuery();
    ctx.reply('/cekbio 628xxx.');
});

bot.action('pairv2', (ctx) => {
    ctx.answerCbQuery();
    ctx.reply('/pair 628xxx');
});


bot.command('pair', checkAccess('owner'), async (ctx) => {
...
bot.command('ctxt', checkAccess('premium'), async (ctx) => {
...
bot.command(['add', 'del'], checkAccess('owner'), (ctx) => {
...
bot.command('list', checkAccess('owner'), (ctx) => {
